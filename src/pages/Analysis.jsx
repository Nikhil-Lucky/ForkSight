import { Activity, AlertTriangle, Boxes, Check, Clipboard, Download, ExternalLink, FileClock, FileSearch, FlaskConical, History, LayoutDashboard, Network, Radar, Scale, ShieldCheck, Workflow } from 'lucide-react'
...
const tabs = [
  ...
  { name: 'Bug Resurrection', icon: History }, { name: 'Deep Evidence', icon: FileSearch }, { name: 'Blast Radius', icon: Network }, { name: 'What-If Simulator', icon: FlaskConical },
...
function DownloadButton({ text }) {
  const download = () => {
    ...
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'forksight-report.md'; anchor.click()
    URL.revokeObjectURL(url)
  }
  return <button className="button button-small copy-button" type="button" onClick={download}><Download size={14}/>Download Markdown Report</button>
}
...
const buildPacks = scan => {
  ...
  const testCode = scan.futureRisks.map((risk, index) => {
    const file = risk.affectedAreas[0] || scan.deepEvidence?.[0]?.path || ''
    const category = risk.category.toLowerCase()
    const purpose = category.includes('rate') ? 'Handle a GitHub 429 response' : category.includes('cache') ? 'Reject stale cached data' : category.includes('api') || category.includes('error') ? 'Handle an upstream API failure' : category.includes('security') || category.includes('validation') || category.includes('input') ? 'Reject malformed or untrusted input' : 'Preserve expected behavior for the proposed change'
    const code = `// Suggested test code — not executed. Adapt imports and API to this repository.\ndescribe(${JSON.stringify(risk.category)}, () => {\n  it(${JSON.stringify(purpose)}, async () => {\n    // Arrange the boundary condition for ${file || 'the affected module'}\n    // Act through the module's public export, then assert a safe explicit result\n    expect(true).toBe(true)\n  })\n})`
    return { id: `test-${index}`, purpose, relatedRisk: risk.title, file, code }
  })
  scan.resurrectionRisks.slice(0, 3).forEach((risk, index) => testCode.push({ id: `regression-${index}`, purpose: `Preserve the behavior fixed by ${risk.oldCommit.slice(0, 7)}`, relatedRisk: risk.title, file: risk.files[0] || '', code: `// Suggested test code — not executed. Adapt imports and API to this repository.\ndescribe('regression ${risk.oldCommit.slice(0, 7)}', () => {\n  it('preserves the historical fix', async () => {\n    // Reproduce: ${risk.oldFixDescription.replace(/\n/g, ' ')}\n    expect(true).toBe(true)\n  })\n})` }))
  return { affected, guardrails, focused, regressions, testCode }
}
...
function reportFor(scan, packs, rows) {
  const failure = rows.map(({ risk, historical }) => `- ${scan.change} → ${(risk?.affectedAreas || historical?.files || []).join(', ') || 'No evidence-backed area'} → ${risk?.title || 'Potential resurrection risk'} → ${historical ? `${historical.oldCommit.slice(0, 7)} ${historical.oldFixDescription}` : 'No Historical Match'} → ${risk?.prevention || historical?.prevention}`).join('\n') || '- No evidence-backed failure path generated'
  const pr = scan.pullRequest ? `\n**Pull request:** #${scan.pullRequest.number} ${scan.pullRequest.title} (${scan.pullRequest.baseBranch} ← ${scan.pullRequest.headBranch})\n**PR author:** @${scan.pullRequest.author}; +${scan.pullRequest.additions} / -${scan.pullRequest.deletions}` : ''
  const deep = list((scan.deepEvidence || []).map(e => `${e.path}:${e.startLine}-${e.endLine}${e.symbols.length ? ` — ${e.symbols.join(', ')}` : ''}\n\n\`\`\`\n${e.snippet}\n\`\`\``))
  const tests = packs.testCode.map(test => `### ${test.purpose}\n- Related risk: ${test.relatedRisk}\n- Related file: ${test.file || 'Not identified'}\n\n\`\`\`javascript\n${test.code}\n\`\`\``).join('\n\n') || 'No evidence-backed code suggestion generated.'
  return `# ForkSight Analysis Report\n\n**Repository:** ${scan.repository.fullName}${pr}\n**Proposed change:** ${scan.change}\n**Overall predicted risk:** ${scan.overallRisk}\n**Inspected:** ${scan.tree.paths.length} paths, ${scan.recentCommits.length} commits\n\n## Future Risks\n${list(scan.futureRisks.map(r => `${r.title} (${r.severity}, ${percent(r.confidence)}): ${r.explanation}`))}\n\n## Bug Resurrection\n${list(scan.resurrectionRisks.map(r => `${r.oldCommit.slice(0, 7)} — ${r.oldFixDescription}; ${r.prevention}`))}\n\n## Deep Evidence\n${deep}\n\n## Failure Path\n${failure}\n\n## Blast Radius\n${list(packs.affected)}\n\n## Guardrail Pack\n${list(packs.guardrails.map(item => `[ ] ${item}`))}\n\n## Test Pack / generated test suggestions\n**Suggested test code — not executed.**\n\n${tests}\n\n> ForkSight uses deterministic heuristics over bounded public GitHub evidence. Findings are potential, not proof or confirmed defects. Repository code and suggested tests were not executed.`
}
...
export default function Analysis() {
  ...
  const resetWorkspaceScroll = () => {
    ...
    // The document is the scroll container today. Resetting main as well keeps
    // desktop and mobile correct if the workspace becomes independently scrollable.
    const scrollingElement = document.scrollingElement
    const previousBehavior = scrollingElement?.style.scrollBehavior
    if (scrollingElement) scrollingElement.style.scrollBehavior = 'auto'
    window.scrollTo(0, 0)
    if (scrollingElement) scrollingElement.style.scrollBehavior = previousBehavior
  }
  ...
  useLayoutEffect(() => {
    if (!userSelectedSection.current) return
    userSelectedSection.current = false
    resetWorkspaceScroll()
  }, [active])

  const selectSection = name => {
    userSelectedSection.current = true
    if (name === active) resetWorkspaceScroll()
    else setActive(name)
  }
  ...
  let content
  if (active === 'Overview') content = <>{scan.pullRequest && <section className="repo-summary"><div><span className="eyebrow">Pull request #{scan.pullRequest.number}</span><h2>{scan.pullRequest.title}</h2><p>{scan.pullRequest.body || 'No pull request description provided.'}</p><a className="commit-link" href={scan.pullRequest.url} target="_blank" rel="noreferrer">View pull request <ExternalLink size={13}/></a></div><dl><div><dt>Author</dt><dd>@{scan.pullRequest.author}</dd></div><div><dt>Branches</dt><dd>{scan.pullRequest.baseBranch} ← {scan.pullRequest.headBranch}</dd></div><div><dt>Changes</dt><dd>+{scan.pullRequest.additions} / -{scan.pullRequest.deletions} · {scan.pullRequest.changedFiles} files</dd></div></dl></section>}<section className="repo-summary"><div><span className="eyebrow">Repository summary</span><h2>{scan.repository.fullName}</h2><p>{scan.repository.description || 'No repository description provided.'}</p></div><dl><div><dt>Default branch</dt><dd>{scan.repository.defaultBranch}</dd></div><div><dt>Language</dt><dd>{scan.repository.language}</dd></div><div><dt>Tree scope</dt><dd>{scan.tree.truncated ? 'Bounded / truncated' : 'Complete within inspected directories'}</dd></div></dl></section>{summary}<section className="results-section"><h2>Evidence snapshot</h2><div className="two-columns"><div><h3>Potential future risks</h3>{scan.futureRisks.slice(0,3).map(r => <RiskCard risk={r} key={r.title}/>)}{!scan.futureRisks.length && <p className="no-findings">This mode was not selected or no supported risk signal was found.</p>}</div><div><h3>Historical fixes & overlap</h3>{scan.resurrectionRisks.slice(0,3).map(r => <ResurrectionCard risk={r} key={r.oldCommit}/>)}{!scan.resurrectionRisks.length && <p className="no-findings">No historical overlap was detected in the bounded recent history.</p>}</div></div></section></>
  ...
  if (active === 'Deep Evidence') content = <section className="results-section"><span className="eyebrow">Bounded public source inspection</span><h2>Deep Evidence</h2><p className="section-note">Relevant source was read as text from GitHub and never executed. Locations are approximate.</p><div className="cards-grid">{(scan.deepEvidence || []).map(e => { const risk = scan.futureRisks.find(r => r.affectedAreas.includes(e.path)); const fix = scan.resurrectionRisks.find(r => r.files.includes(e.path)); const patch = fix?.patches?.find(p => p.path === e.path); return <article className="finding-card deep-card" key={e.path}><div className="finding-top"><code>{e.path}:{e.startLine}-{e.endLine}</code><a href={e.url} target="_blank" rel="noreferrer">Source <ExternalLink size={12}/></a></div><h3>{e.symbols.length ? e.symbols.join(', ') : 'Relevant source signals'}</h3><pre><code>{e.snippet}</code></pre><div className="finding-action"><strong>Evidence chain</strong><p>Proposed change → {e.path}{e.symbols[0] ? ` / ${e.symbols[0]}` : ''} → {risk?.title || 'review boundary behavior'} → {fix ? `${fix.oldCommit.slice(0,7)} ${fix.oldFixDescription}` : 'no matched historical patch'} → {risk?.prevention || fix?.prevention || 'Review and add focused regression coverage.'}</p>{patch && <><strong>Historical patch excerpt</strong><pre><code>{patch.patch}</code></pre></>}</div></article>})}</div>{!scan.deepEvidence?.length && <p className="no-findings">No suitable text source file was found in the bounded scan.</p>}</section>
  ...
  if (active === 'Test Pack') content = <section className="results-section"><div className="section-title-row"><div><span className="eyebrow">Suggested test code — not executed.</span><h2>Test Pack</h2></div><CopyButton label="Copy All Test Code" text={packs.testCode.map(t => t.code).join('\n\n')}/></div><div className="cards-grid">{packs.testCode.map(test => <article className="finding-card" key={test.id}><h3>{test.purpose}</h3><p><strong>Related risk:</strong> {test.relatedRisk}</p><p><strong>Related file:</strong> <code>{test.file || 'Not identified'}</code></p><pre><code>{test.code}</code></pre><CopyButton label="Copy Test" text={test.code}/></article>)}</div>{!packs.testCode.length && <p className="no-findings">No evidence-backed test suggestion was generated.</p>}</section>
  return <div className="workspace"><aside className="workspace-sidebar"><Brand/><div className="workspace-label">ANALYSIS WORKSPACE</div><nav>{tabs.map(({name, icon: Icon}) => <button className={active === name ? 'active' : ''} onClick={() => selectSection(name)} key={name}><Icon size={17}/><span>{name}</span></button>)}<Link className="sidebar-history" to="/history"><History size={17}/><span>Scan History</span></Link></nav><div className="side-status"><span className="live-dot"/><div><strong>Analysis complete</strong><small>{shortDate(scan.createdAt)}</small></div></div></aside><main className="workspace-main" ref={workspaceMainRef}><header className="workspace-header"><div><small>REPOSITORY / {scan.repository.fullName.toUpperCase()}</small><h1>{active}</h1></div><div className="workspace-actions"><span className={`risk-badge ${scan.overallRisk}`}><AlertTriangle size={12}/> {scan.overallRisk} risk</span><CopyButton label="Copy Report" text={report}/><DownloadButton text={report}/><Link to="/analyze" className="button button-small">New analysis</Link></div></header><div className="workspace-content"><div className="context-bar"><div><Boxes/><span><small>REPOSITORY</small>{scan.repository.fullName} · {scan.repository.language}</span></div><div><FileClock/><span><small>PROPOSED CHANGE</small>{scan.change}</span></div><div><Activity/><span><small>EVIDENCE</small>{scan.recentCommits.length} commits · {scan.tree.paths.length} paths</span></div></div>{content}</div></main></div>
}
